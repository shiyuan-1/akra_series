...
def get_wk_pol(mask, lmax, niter=3, nside_out=None):
    ...
    for ii in tqdm(range(lm_size)):
        ...
        # _, map_g1, map_g2 = hp.alm2map([np.zeros_like(a_Blm), a_Elm, a_Blm], nside, pol=True, lmax=lmax)
...
class KappaRec_sphere():
    def __init__(self, gamma1, gamma2, mask=None,nside_out=None, choSolve=True, b_inv=None, lmax=None, verbose=True):
        ...
        self.yy = np.vstack((get_fullalm(self.a2lm_mask), get_fullalm(self.am2lm_mask))).astype("complex64")
        if self.verbose:
    ...
    def sphere_AKRA(self, nosh=False, b_inv=None, matrix_a=None, **kwargs):
        if matrix_a is None and self.b_inv is None:
            self.get_psf()
        ...
        lm_size = hp.Alm.getsize(self.lmax)
    ...
    def get_psf(self, nside_M=None):
        ...
        self.matrix_a = np.vstack([M_a2lm, M_am2lm]).astype("complex64")
        ...
        self.psf = np.matmul(self.matrix_a.conj().T, self.matrix_a).astype("complex64")
